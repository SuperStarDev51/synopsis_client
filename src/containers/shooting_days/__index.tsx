...
export const ShootingDays: React.FunctionComponent<Props> = React.memo((props: Props) => {
	...
	const [next, setNext] = React.useState(ITEMS_PER_PAGE);
	  ...
	  const handleScrollTo = (index: number) => {
		...
		setTimeout(() => {
			...
			const element = document.querySelector('.list-inner div[data-rbd-droppable-id="'+ index +'"]');
	...
	const handleScroll = (e) => {
		...
		if (Math.floor(el.offsetWidth + Math.abs(el.scrollLeft)) >= el.scrollWidth - 20 && !isListPreview) {
			getMoreData();
		}
	...
	useDidUpdateEffect(() => {
		if (isListPreview) {
			...
			setHasMore(true);
			const daysToRender = Math.round(window.innerWidth / 300) + 1; // 400 width of columns
	...
	React.useEffect(() => {
		if( !reorderd && shootingDays && shootingDays.length  ) {
			...
			for (var sdi=0; sdi < payload.length ;sdi++) {
				...
				let shootingDayDuration = 0;

				// loop over scenes of each day
				for(var scene_index=0; scene_index < sd.shooting_day.total_scenes.length ;scene_index++) {
					...
					//breakfast break
					if (scene_index === 0) {
						if (!(!!sd?.shooting_day?.total_scenes?.find(scene => scene?.dayBreak?.type === 'breakfast'))) {
							scene.dayBreak = {type: 'breakfast'};
						}
					} else {

						//lunch break
						if ( shootingDayDuration < 300 ) {
							shootingDayDuration += scene_duration
						} else {

							if (!sd?.shooting_day?.total_scenes?.find(scene => scene?.dayBreak?.type === 'lunch')) {
								scene.dayBreak = {type: 'lunch'};
							}

							// scene.break = {type: 'lunch'};
							console.log('*********')
							console.log('shootingDayDuration: ', shootingDayDuration)
							console.log('scene: ', scene)
							console.log('scene_index, scene.scene_id: ', scene_index, scene.scene_id)
							console.log('*********')
						}

						//dinner
						if ( shootingDayDuration < 600 ) {
							shootingDayDuration += scene_duration
						} else {
							if (!(!!sd?.shooting_day?.total_scenes?.find(scene => scene?.dayBreak?.type === 'dinner'))) {
								scene.dayBreak = {type: 'dinner'};
							}
						}
	...
	return  (
		...
		>
			...
			<Droppable droppableId="shootingDaysDroppable" type="shootingDayItem" direction="horizontal" className="height-100-per">
				{(provided1: DroppableProvided, snapshot1: any): React.ReactElement => (
															...
															{(provided: DroppableProvided, snapshot: any): React.ReactElement => (
																<div ref={provided.innerRef} {...provided.droppableProps}>
																	{sd?.shooting_day?.total_scenes?.map((scene:any, scene_index:number) => {
																		...
																		{scene.dayBreak && sceneBreak(scene.dayBreak, sdi, sd.id)}
