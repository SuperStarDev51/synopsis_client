...
import {addShootingDay, addShootingDays, moveScenesBetweenShootingDays, deleteShootingDay} from './initial-state';
...
export const ShootingDays: React.FunctionComponent<Props> = React.memo((props: Props) => {
				...
				// for(var time_index=0; time_index < sd.shooting_day.total_scenes.length ;time_index++) {
					...
					for(var scene_index=0; scene_index < sd.shooting_day.total_scenes.length ;scene_index++) {
						...
						if( scene && sceneBefore && sceneBefore.timeEnd) {
							...
							// console.log('sceneBefore', sceneBefore)
							// console.log('scene', scene)
                            //
							// console.log('sceneBefore.timeEnd.getTime()', sceneBefore.timeEnd.getTime())
							// console.log('*****')
	...
	const onDragSceneEnd = (result: DropResult): void => {
		...
		if (source.droppableId !== destination.droppableId) {
			...
			if (shootingDayFrom.shooting_day.total_scenes.length === 1) {
				let updatedShootingDays = shootingDays.filter(shootingDay => shootingDay.id !== shootingDayFrom.id);
				dispatch({
					type: ShootingDaysActionTypes.SET_SHOOTING_DAYS,
	...
	const setDates = async (firstDate: Date, daysList, setReorerdFalse) => {
		var businessDays = 0
		...
		if (setReorerdFalse) setReorerd(false)
	}
	...
	const sceneBreak = (scene_break:any, sdi:number, sdId:number, scene:any) => {
		let startTime;
		let endTime;

		if (scene_break.type !== 'breakfast') {
			startTime = moment(scene.timeEnd).format('HH:mm') || scene_break['inside']['start'];
			endTime = moment(scene.timeEnd).add(30, 'minutes').format('HH:mm') || scene_break['inside']['end'];
		} else {
			startTime = scene_break['inside']['start'];
			endTime = scene_break['inside']['end'];
		}
	...
	const getShootingDayStartTime = sd => sd?.shooting_day?.total_scenes[0].break?.inside.start.replace('AM', '').replace('PM', '')
	...
	const getShootingDayDuration = sd => {
		let totalTimeInMin;
		const totalScenesDurationInMin = sd?.shooting_day?.total_scenes.map(scene => scene.scene_duration).reduce((accumulator, currentValue) => accumulator + currentValue);
		const totalBreaksDuration = sd?.shooting_day?.total_scenes.map(scene => {
			//console.log('scene.scene_id: ', scene.scene_id)
			return scene.break?.duration
		}).reduce((accumulator, currentValue) => accumulator + currentValue);
		const totalBreaksDurationInMin = moment.duration(totalBreaksDuration).asMinutes();

		//totalTimeInMin = totalScenesDurationInMin + totalBreaksDurationInMin;
		totalTimeInMin = totalScenesDurationInMin;

		// console.log('*******')
		// console.log('sd.id: ', sd.id)
		// console.log('totalScenesDurationInMin: ', totalScenesDurationInMin)
		// console.log('totalBreaksDuration: ', totalBreaksDuration)
		// console.log('totalBreaksDurationInMin: ', totalBreaksDurationInMin)
		// console.log('totalTimeInMin: ', totalTimeInMin)
		// console.log('*******')

		return moment.utc(moment.duration(totalTimeInMin, "minutes").asMilliseconds()).format("HH:mm")
	}
					...
					<div className="font-medium-1 text-bold-700 mr-05 p-0"><FormattedMessage id="first_shoot"/></div>
					  <div className="ttt d-flex align-items-center bg-light-gray p-05 ml-1">
						 <Icon src={config.iconsPath+"options/calendar.svg"} style={{height: '1rem', width: '1rem'}} className="mr-05"/>
						  ...
						  onChange={(date:any) => { {
							setDates(date, shootingDays, true);
							dispatch({
											...
											>
												<div key={sdi} className={classnames("list height-min-content pt-2",{"p-1": isListPreview, "px-05 width-18-rem min-width-18-rem": !isListPreview})}>
													<div className={classnames("d-flex justify-content-start align-items-center flex-wrap", {"mb-1": isListPreview})} style={{height: isListPreview ? null : '4.9rem'}}>
														...
														{sd.date ? (
															<div className={classnames("d-flex align-items-center bg-light-gray font-medium-1 p-05", {"ml-1": isListPreview, "ml-05": !isListPreview})}>
																<Icon src={config.iconsPath+"options/calendar.svg"} style={{height: '1rem', width: '1rem'}} className="mr-05"/>
																<select
														...
														{sd?.shooting_day?.total_scenes && sd?.shooting_day?.total_scenes[0]?.break ? (
															<div className={classnames("bg-light-gray d-inline-block p-05",{"ml-1": isListPreview})}>
															{getShootingDayStartTime(sd)} - {getShootingDayEndTime(sd)}
															</div>
															):null}
														{sd?.shooting_day?.total_scenes.length !== 0 && (
															<div className={classnames("bg-light-gray d-flex p-05 ml-1 align-items-center", {"ml-05": !isListPreview})}>
																<Icon src={config.iconsPath + "script/duration_black.svg"} style={{ height: '.8rem', width: '.8rem', marginRight: '.3rem' }} />
																{getShootingDayDuration(sd)}
															</div>
														)}
														{sd?.shooting_day?.total_scenes.length > 0 ? (
															<>
															...
															<span className="ml-05 text-lowercase">({sd.shooting_day.total_scenes.length} <FormattedMessage id='scenes'/>)</span>
															</>
														):null}
														{sd?.shooting_day?.total_scenes.length === 0 ? (
															<div
																...
																onClick={() => {
																	deleteShootingDay(sd.id)
																	dispatch({
																		type: ShootingDaysActionTypes.DELETE_SHOOTING_DAY,
																		payload: {
																			shootingDayId: sd.id
																		},
																	});
															...
															{(provided: DroppableProvided, snapshot: any): React.ReactElement => (
																<div ref={provided.innerRef} {...provided.droppableProps}>
																	{sd?.shooting_day?.total_scenes?.map((scene:any, scene_index:number) => {
																		...
																		return (
																		<>
																		{scene.break && !isBreakAfter ? sceneBreak(scene.break, sdi, sd.id, scene) : null}
																			...
																			>
																				<BreakDownScene
																					...
																					changeScenePropValue={(
																						...
																						if (['time_id', 'prepare', 'one_shot', 'reshoot', 'scene_duration'].includes(field)) {
																							setReorerd(false);
																						}
																						changeScenePropValue(
																							value,
																					...
																					onDragEnd={(r: any) => onDragEnd(r, scene_index, scene.chapter_number, sdi, scene.time_id)}
																					onDelete={deleteRow}
